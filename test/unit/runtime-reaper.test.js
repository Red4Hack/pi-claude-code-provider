...
function spawnDetached(cwd) {
  const child = spawn(process.execPath, nodeFixtureArgs(["-e", "setInterval(() => {}, 1000)"]), {
    cwd,
    detached: true,
    stdio: "ignore",
  });
...
test("a live process that does not run in the recorded directory is never signalled", async (t) => {
  if (process.platform !== "linux") return t.skip("ownership proof reads /proc");
  ...
  try {
    ...
    // Runs elsewhere: this is what a reused process identifier looks like, and
